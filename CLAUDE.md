...
| `js/import-export.js` | JSON bulk import/export via File API. **Export format:** `{ "transactions": [...] }` — wrapper object so the schema can be extended (e.g. with settings) without breaking compatibility. **Import** accepts both the new wrapper format and the legacy bare-array format `[...]`; if `jsonData.transactions` is not an array AND `jsonData` itself is not an array, `bulkAdd` is skipped. |
