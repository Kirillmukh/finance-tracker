...
1. **Pre-selected on the input page** — `applyDefaultRate()` in `app.js` sets `#rate-select` value when the user navigates to the input page (nav click listener) or when the app loads with `page === 'input'`.
